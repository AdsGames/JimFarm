...
// Ticker
TileMap::~TileMap() {}
volatile int TileMap::ticks = 0;
void TileMap::tick_counter() {
  ticks++;
}
END_OF_FUNCTION(tick_counter)

// Init ticker to 20 ticks per second
void TileMap::init_ticker() {
  LOCK_VARIABLE (ticks);
 ...
 */
// Draw bottom tiles
void TileMap::draw (BITMAP *tempBuffer) {
  // Draw tiles
...
void TileMap::drawForeground (BITMAP *tempBuffer) {
...
Tile *TileMap::tile_at (int positionX, int positionY, bool foreground) {
...
void TileMap::place_tile (Tile* newTile, bool foreground) {
  if (newTile) {
    if (foreground)
...
bool TileMap::place_tile_safe (Tile* newTile, bool foreground, int opposite_layer_id) {
...
// Replace tile on map
void TileMap::replace_tile (Tile *oldTile, int newID, bool foreground) {
  if (oldTile) {
...
void TileMap::remove_tile (Tile* newTile, bool foreground) {
  if (newTile) {
    std::vector<Tile*> *layer = &map_tiles;
    if (foreground)
      layer = &map_tiles_foreground;
...
Item *TileMap::item_at (int positionX, int positionY) {
  for (unsigned int i = 0; i < map_items.size(); i++)
...
// Place item on map
void TileMap::place_item (Item* newItem, int x, int y) {
  if (newItem) {
...
void TileMap::remove_item (Item *newItem) {
  if (newItem != NULL) {
    for (unsigned int i = 0; i < map_items.size(); i++) {
 ...
 */
// Interact with
void TileMap::interact (int inter_x, int inter_y, Item *inHand) {
...
// Scroll
void TileMap::scroll (int player_x, int player_y) {
  // Scroll x
